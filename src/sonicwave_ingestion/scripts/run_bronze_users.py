...
from sonicwave_ingestion.pipelines import run_bronze_users
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Bronze users ingestion pipeline for a local snapshot."
    )
    parser.add_argument(
        "--source",
        dest="source_path",
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    spark = SparkSession.builder.appName("bronze-users-pipeline").getOrCreate()
    try:
        run_bronze_users(
            spark=spark,
